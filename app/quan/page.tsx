...
export default function StoreDashboard() {
  ...
  // State Tự Upload Ảnh Món từ Thiết bị (Staff Upload)
  ...
  // Xử lý nhân viên tự upload ảnh món từ máy
  const handleFileUpload = async (file: File) => {
    if (!file) return;

    const allowedTypes = ["image/jpeg", "image/png", "image/webp", "image/gif"];
    if (!allowedTypes.includes(file.type)) {
      setUploadError("Chỉ chấp nhận ảnh định dạng JPG, PNG, WEBP, GIF.");
      return;
    }
    ...
    // Xem trước ảnh ngay lập tức trên máy khách
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        setUploadError(data.error || "Không thể tải ảnh lên máy chủ");
      }
    } catch (err: any) {
          ...
          <div className="min-h-full flex items-center justify-center p-3 sm:p-4">
            <div className="relative bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in text-slate-100">
              ...
              <form onSubmit={handleSaveMenuItem} className="p-4 sm:p-6 space-y-4">
                ...
                {/* Ảnh món ăn (Tự upload từ thiết bị & Link URL) */}
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">
                      Ảnh món ăn <span className="text-orange-500">*</span>
                    </label>
                    <button
                      type="button"
                      onClick={() => setShowUrlInput(!showUrlInput)}
                      className="text-[11px] text-orange-400 hover:text-orange-300 transition-colors underline"
                    >
                      {showUrlInput ? "Ẩn nhập link URL" : "Dán link URL bên ngoài"}
                    </button>
                  </div>

                  {/* Khu vực Upload Tự động từ Thiết bị */}
                  <div className="space-y-2.5">
                    ...
                    <div
                      ...
                      className={`relative border-2 border-dashed rounded-2xl p-3.5 transition-all cursor-pointer ${
                        isUploadingImage
                          ? "border-orange-500/50 bg-orange-950/20 cursor-wait"
                          : "border-slate-700 hover:border-orange-500 bg-slate-950/70 hover:bg-slate-950"
                      }`}
                    >
                      {isUploadingImage ? (
                        <div className="py-4 flex flex-col items-center justify-center gap-2 text-orange-400">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 text-left min-w-0">
                            <p className="text-xs font-bold text-white mb-1 flex items-center gap-1.5">
                              ...
                              <span>Ảnh món đã sẵn sàng</span>
                            </p>
                            <p className="text-[11px] text-slate-400 mb-2 truncate max-w-[220px]">
                              {formImage.startsWith("blob:") ? "Ảnh từ thiết bị" : formImage}
                            </p>
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                              ...
                              <button
                                ...
                                className="px-2.5 py-1 rounded-lg bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 text-xs font-medium transition-colors"
                              >
                                Xóa ảnh
                              </button>
                            </div>
                        ...
                        <div className="py-3.5 flex flex-col items-center justify-center gap-1 text-slate-400 text-center">
                          <div className="p-2.5 bg-slate-900 rounded-xl text-orange-400 mb-0.5 border border-slate-800">
                            <Upload className="w-5 h-5" />
                          </div>
                          <p className="text-xs font-bold text-slate-200">
                            Bấm để chụp ảnh hoặc chọn ảnh từ máy
                          </p>
                    ...
                    {/* Upload Error Alert */}
                    {uploadError && (
                      <div className="bg-rose-950/40 border border-rose-800 text-rose-300 text-xs p-2.5 rounded-xl font-medium flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4 shrink-0 text-rose-400" />
                        <span>{uploadError}</span>
                      </div>
                    )}

                    {/* Secondary URL input (Expandable) */}
                    {showUrlInput && (
                      <div className="pt-1">
                        ...
                        <input
                          ...
                          className="w-full px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs font-medium focus:border-orange-500 focus:outline-none text-white transition-all"
                        />
                      </div>
                    )}
                  </div>
